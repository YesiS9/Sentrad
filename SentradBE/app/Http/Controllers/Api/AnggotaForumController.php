...
class AnggotaForumController extends Controller
{
    ...
    // Method untuk keluar dari forum
    // public function outForum(Request $request)
    // {
    //     // Validasi input
    //     $request->validate([
    //         'forum_id' => 'required|exists:forums,id',
    ...
    //     // Cek apakah seniman memang anggota forum
    //     $anggotaExist = AnggotaForum::where('forum_id', $request->forum_id)
    //                                 ->where('anggota_id', $request->seniman_id)
    //                                 ->first();

    //     if (!$anggotaExist) {
    //         return response()->json(['message' => 'Seniman belum menjadi anggota forum ini'], 404);
    //     }

    //     // Hapus anggota dari forum (Soft Delete)
    //     $anggotaExist->delete();
    ...
    {
        try {
            $anggota = AnggotaForum::whereNull('deleted_at')->find($id);
