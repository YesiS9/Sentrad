<template>
  ...
  const loadPenilaianData = async (id) => {
    try {
      ...
      if (response.status === 200) {
        ...
        selectedRubriks.value = data.rubrik_penilaians.map((rubrik) => rubrik.id); // Use IDs
        data.rubrik_penilaians.forEach((rubrik) => {
          rubrikSkors[rubrik.id] = rubrik.skor; // Assuming each rubrik has an ID and score
        });
      } else {
  ...
  const handleRubrikChange = () => {
    formData.rubrik_penilaians = selectedRubriks.value.map((rubrikId) => {
      ...
      return {
        id: rubrikId,
        nama_rubrik: rubrik ? rubrik.nama_rubrik : '', // Get the name of the rubric
