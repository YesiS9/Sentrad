<template>
  ...
  editedComment.value = komen.isi_komenForum; // Changed from isi_komenProyek to isi_komenForum
  showMenu.value = null;
};
...
const saveEditComment = async (komen) => {
  try {
    const response = await axios.put(`/komenForum/${komen.id}`, {
      isi_komenForum: editedComment.value, // Changed from isi_komenProyek to isi_komenForum
    });
    if (response.status === 200 && response.data.status === 'success') {
      komenForum.value[editIndex.value].isi_komenForum = editedComment.value; // Changed from komenProyek to komenForum
...
const deleteComment = async (komen) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.delete(`/komenForum/${komen.id}`); // Changed from komenProyek to komenForum
      if (response.status === 200 && response.data.status === 'success') {
        komenForum.value = komenForum.value.filter((k) => k.id !== komen.id); // Changed from komenProyek to komenForum
